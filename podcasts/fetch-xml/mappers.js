...
const feedXmlToEpisode = (xmlChannel, epi, uri) => {
  const channelImageUri =
    get(xmlChannel, "image.url") || get(xmlChannel, "itunes.image");
...
const getEpisodeDescription = (episode) => {
  ...
  return description.length > itunesSubTitle.length
    ? description
    : itunesSubTitle;
};
...
const getEpisodeSummary = (epi) => {
  const str =
    get(epi, "itunes.subtitle") ||
    get(epi, "itunes.summary") ||
    epi.description ||
    "";
